...
class Analyzer():
    ...
    def LOOCV_gesture_mappings(self):
        similarities = []
        for gid in tqdm(list(self.complete_gesture_data.keys())):
            ...
            max_sim = 0
            s_max_id = 0
            for gid2 in list(self.complete_gesture_data.keys()):
                if gid2 == gid:
                    continue
    ...
    def get_avg_gesture_distance(self):
        vectors = [self.complete_gesture_data[k]['feature_vec'] for k in list(self.complete_gesture_data.keys())]
        dists = []
    ...
    def get_closest_gesture_by_sentence(self, gid):
        ...
        max_sim = 0
        max_gesture_id = 0
        for comp in list(self.complete_gesture_data.keys()):
            if comp == gid:
                continue
    ...
    def get_avg_closest_gesture_by_sentences(self):
        avg_dists = []
        for g in tqdm(list(self.complete_gesture_data.keys())):
    ...
    def plot_dist_s_sprime_g_gprime(self):
        ...
        for i in range(len(keys)):
            s = self.complete_gesture_data[keys[i]]['sentence_embedding']
            for j in tqdm((list(range(i, len(keys))))):
                if i == j:
                    continue
                s_j = self.complete_gesture_data[keys[j]]['sentence_embedding']
    ...
    def get_semantically_distinguishable_gestures(self, take_n=0):
        ...
        totals = []
        keys = list(self.complete_gesture_data.keys())
        for i in tqdm(list(range(take_n))):
            k = keys[i]
